...
public partial class SKCanvasDrawings
{
	extension(SKCanvas @this)
	{
		...
		)
		{
			...
			var trailPath = CapsuleTrailDrawer.BuildTrailFromCapsuleToCells(capsulePath, trailCellPoints, cellSize);
			...
			CapsuleTrailDrawer.DrawArrowAtEnd(@this, p0, p1, arrowLength, arrowHalfAngleDegrees, linePaint);
...
file static class CapsuleTrailDrawer
{
	...
	public static SKPath BuildCapsulePath(ReadOnlySpan<SKPoint> capsuleCells, float cellSize, Scale capsuleSizeScale)
	{
	...
	public static SKPath BuildTrailFromCapsuleToCells(SKPath capsulePath, ReadOnlySpan<SKPoint> trailCells, float cellSize)
	{
		var path = new SKPath();
	...
	public static void DrawArrowAtEnd(
		SKCanvas canvas,
	...
	)
	{
		...
		var angle = arrowHalfAngleDegrees * MathF.PI / 180;
		...
		var left = new SKPoint(endPoint.X + leftDir.X * arrowLength, endPoint.Y + leftDir.Y * arrowLength);
		var right = new SKPoint(endPoint.X + rightDir.X * arrowLength, endPoint.Y + rightDir.Y * arrowLength);
	...
	private static bool IsHorizontal(ReadOnlySpan<SKPoint> cells)
		=> cells[0] is var first && (cells.Length < 2 || cells.All(c => MathF.Abs(c.Y - first.Y) < 1E-3F));

	private static SKPoint ToCenter(SKPoint cell, float cellSize)
		=> new(cell.X + cellSize / 2, cell.Y + cellSize / 2);

	private static SKPoint FindBorderPointByBinarySearch(SKPath capsulePath, SKPoint inside, SKPoint outside)
	{
	...
	private static SKPoint Rotate(SKPoint v, float radians)
	{
