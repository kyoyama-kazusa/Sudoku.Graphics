...
/// <summary>
/// Represents a cell group trailed capsule satellite mark item.
/// </summary>
public sealed record CellGroupTrailedCapsuleSatelliteMarkItem : CellGroupMarkItem
{
	/// <inheritdoc/>
	public override ItemType Type => ItemType.CellGroup_CapsuleWithSatellite;
...
/// <summary>
/// <para>Provides a way to draw capsules and trails.</para>
/// <para>
/// Although this type is named same as another file-local type defined in <see cref="CellGroupTrailedCapsuleArrowMarkItem"/>,
/// they are actually different on drawing trails.
/// </para>
/// </summary>
/// <seealso cref="CellGroupTrailedCapsuleArrowMarkItem"/>
file static class CapsuleTrailDrawer
{
	...
	public static SKPath BuildTrailPath(SKPath capsulePath, ReadOnlySpan<SKPoint> trailCells, float cellSize)
	{
		...
		for (var i = 1; i < trailCells.Length; i++)
		{
			var currentCenter = toCenter(trailCells[i], cellSize);

			// The last point should stop at the border of the last cell instead of stopping at the center.
			if (i == trailCells.Length - 1)
			{
				var prevCenter = toCenter(trailCells[i - 1], cellSize);
				path.LineTo(findBorderPointOnCell(currentCenter, prevCenter, cellSize));
			}
			else
			{
				path.LineTo(currentCenter);
			}
		}
		...
		static SKPoint findBorderPointOnCell(SKPoint currentCenter, SKPoint previousCenter, float cellSize)
		{
			...
			// the smaller scale needed to reach one of the 4 sides.
		...
		static SKPoint findBorderPointByBinarySearch(SKPath capsulePath, SKPoint inside, SKPoint outside)
		{
			...
			for (var i = 0; i < 28; i++)
			{
				...
				(capsulePath.Contains(p.X, p.Y) ? ref low : ref high) = mid;
			}
