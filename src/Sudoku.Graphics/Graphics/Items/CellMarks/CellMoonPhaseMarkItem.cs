...
public sealed record CellMoonPhaseMarkItem : CellMarkItem
{
	...
	protected internal override void DrawTo(Canvas canvas)
	{
		...
		var cx = x + cellSize / 2;
		var cy = y + cellSize / 2;
		var radiusOfMainCircle = SizeScale.Measure(cellSize / 2);
		...
		switch (Phase)
		{
			...
			case MoonPhase.Full:
			{
				backingCanvas.DrawCircle(cx, cy, radiusOfMainCircle, fillPaint);
				if (strokeWidth != 0)
				{
					backingCanvas.DrawCircle(cx, cy, radiusOfMainCircle, strokePaint);
				}
				break;
			}
			...
			case MoonPhase.UpperHalf_Curve or MoonPhase.LowerHalf_Curve:
			{
				...
				if (crescentPath is not null)
				{
					...
					backingCanvas.DrawPath(crescentPath, fillPaint);
					if (strokeWidth != 0)
					{
						backingCanvas.DrawPath(crescentPath, strokePaint);
					}
				}
				else
				{
					...
					backingCanvas.DrawPath(fallback, fillPaint);
					if (strokeWidth != 0)
					{
						backingCanvas.DrawCircle(cx, cy, radiusOfMainCircle, strokePaint);
					}
