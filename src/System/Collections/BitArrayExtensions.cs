...
public static class BitArrayExtensions
{
	...
	/// <summary>
	/// Provides extension members on <see cref="BitArray"/> instanaces.
	/// </summary>
	/// <param name="this">The current instance.</param>
	extension(BitArray @this)
	{
		/// <summary>
		/// Indicates the number of bits set.
		/// </summary>
				...
				static unsafe int popcnt_Avx2(byte[] data)
				{
					...
					fixed (byte* ptr = data)
					{
						for (; i + 32 <= length; i += 32)
						{
							...
							var popLo = Ssse3.Shuffle(lookup, lo.GetLower()); // need 128-bit lanes
							var popLoHi = Ssse3.Shuffle(lookup, lo.GetUpper());

							// high nibble
							var hi = Avx2.And(Avx2.ShiftRightLogical(v.AsUInt16(), 4).AsByte(), lowMask);
							var popHi = Ssse3.Shuffle(lookup, hi.GetLower());
							var popHiHi = Ssse3.Shuffle(lookup, hi.GetUpper());

							var sumLo = Sse2.Add(popLo, popHi);
							var sumHi = Sse2.Add(popLoHi, popHiHi);
